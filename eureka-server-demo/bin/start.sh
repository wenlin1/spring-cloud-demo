...
SERVER_PORT=9010
SERVER_NAME=eureka-service-demo-1.0-SNAPSHOT.jar
...
PROFILES=$1
if [ ! -n "$PROFILES" ]; then
    echo "ERROR: the profiles is not null!"
    echo "start [profiles]"
    exit 1
fi

if [ ! -f "$CONF_DIR/application-$PROFILES.properties" ]; then
    echo "ERROR: the $PROFILES does not exists! config file $CONF_DIR/application-$PROFILES.properties not exists!"
    exit 1
fi
...
nohup $JAVA_HOME/java $JAVA_OPTS -jar $SERVER_DIR/$SERVER_NAME --server.port=$SERVER_PORT --spring.config.location=$CONF_DIR --spring.profiles.active=$PROFILES > $STDOUT_FILE 2>&1 &
